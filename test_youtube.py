...
def infer2(model, postprocessors, device, output_path, dm):
    ...
    for samples, _targets, image_id, *_ in data_loader_train:
        ...
        print("Len pred_boxes: ", len(predicted_targets["pred_boxes"][0]))
        #print(predicted_targets["pred_boxes"][0])
        # ~ print(predicted_targets.keys()) # pred_logits, pred_boxes, aux_outputs
        
        # horrible code untill the end of the function --> It does not work
        ...
        for h5_sample_name in train_ds:  # for every h5 file in the chosen path 
            if h5_sample_name["record"] == image_id[0]:
                ...
                plt.plot(x,y)
                print(predicted_targets["pred_boxes"][0])
                for i in range(0,len(predicted_targets["pred_boxes"][0])): # iterate ove r the number of found events
                    x = predicted_targets["pred_boxes"][0][i][0] # extract the mean value of the event
                    ymin = 0
                    ymax = 1
                    plt.vlines(x*600, ymin, ymax)
                plt.show()
